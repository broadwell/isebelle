...
1. If you'll be working with Icelandic, Frisian or Low German texts, be sure to run `just build-[icelandic|frisian|low-german]-dictionary` once for each language.
1. Each collection should be a sub-folder of the main data folder identified by `STORIES_SRC_FOLDER` in `.env`. The actual story files should be in individual .xml files, with the filename providing the story ID, in the following folder structure: `[collection_folder]/records/isebel/`. It is preferred to use underscores (`_`) rather than spaces in the file and folder names. Ideally, each collection should contain stories in a single language. The name of the collection folder should be supplied as the `[COLLECTION_NAME]` argument to the scripts below.
